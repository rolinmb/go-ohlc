...
def fetchSeriesData(ticker, logging=True, ts_csv_out=None):
    if (ts_csv_out is None or ts_csv_out.strip() == '' or ts_csv_out.strip() == '.csv' or not ts_csv_out.endswith('.csv')):
        ...
        print('<fetch_data.py fetchSeriesData()> NOTICE: Invalid/no .csv filename entered for AlphaVantage Time Series data; using default option ''%s'''%ts_csv_out)
    ...
    with urllib.request.urlopen(url) as source:
        try: # Attempt to read JSON, if no JSON then maybe 404 error or other type of error
            ...
            exit(f'<fetch_data.py fetchSeriesData()> ERROR! {e}: Couldn''t fetch the AlphaVantage API URL => calling sys.exit()')
        try: # If JSON loaded correctly, isolate the Time Series OHLC data from the AlphaVantage API
            data = data['Time Series (Daily)']
        except KeyError:
            exit(f'<fetch_data.py fetchSeriesData()> KEY ERROR! {ticker} is not valid to query on AlphaVantage => calling sys.exit()')
        if logging:
            print(f'<fetch_data.py fetchSeriesData()> urllib.request.urlopen() successfully navigated to\n\t{url}')
        ...
        del(df)
        if logging:
            print('<fetch_data.py fetchSeriesData()> Successfully created/refreshed ''%s'''%ts_csv_out)
...
if __name__ == '__main__':
    ...
    print(f'\n<fetch_data.py __main__> Total Execution Time: {str(round(time.time() - t_start, 2))} seconds')